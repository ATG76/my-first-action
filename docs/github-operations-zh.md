...
本文记录 OpenCode Personal Reverse Assistant 仓库的 Git/GitHub 操作流程。
...
| 已验证功能基线 | 以对应提交的测试输出和 Actions 记录为准 |
| 离线测试 | 执行 `node --test reverse-control\tests\reverse-control.test.mjs` 后确认 |
| GitHub Actions | 以仓库 Actions 页面对应提交的记录为准 |
| 部署方式 | `scripts\deploy.ps1` |

不要在文档中手工声明“当前有多少测试通过”或“某个工作区已部署”。这些是运行事实，应由对应提交、终端测试输出和 GitHub Actions 记录证明。以后使用 `git status`、`git log --oneline -5` 和 Actions 页面确认。

严格控制面实验已保存到本地归档分支 `archive/reverse-control-strict-20260717` 的提交 `dc9537d`。它保留用于比较和回溯，不是个人逆向助手的后续开发基线。
...
| 路径 | 用途 |
| --- | --- |
| `plugins/reverse-control.js` | OpenCode 自动加载的个人逆向助手插件入口 |
| `commands/reverse-start.md` | 将自由文本转为轻量任务简报的 `/reverse-start` 命令 |
| `commands/reverse-close.md` | 汇报未解决状态而不阻塞退出的 `/reverse-close` 命令 |
| `reverse-control/` | engine lease、任务简报、source 生命周期和本地状态存储代码 |
